...
import { getLoginQrCodes } from '../api/endpoints';
...
export default function ShowUnusedLoginQrCodesPage() {
  ...
  useEffect(() => {
    const fetchUnusedCodes = async () => {
      try {
        const { data } = await getLoginQrCodes(adminToken!);
  ...
  const handlePrint = () => {
    if (printRef.current) {
      ...
      if (printWindow) {
        printWindow.document.write('<html><head><title>Nicht verwendete Login QR-Codes</title>');
  ...
  if (isLoading) return <p>Lade nicht verwendete Login QR-Codes...</p>;

  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      <h2>Nicht verwendete Authorizierungs QR-Codes</h2>
      ...
      {unusedCodes.length === 0 && !errorMessage ? (
        <p>Keine nicht verwendeten Authorizierungs QR-Codes vorhanden.</p>
      ) : (
        <div>
